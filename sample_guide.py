...
def sample(opt):
    ...
    if local_rank == 0:
        if opt.model_type == 'EDM':
            gen_dir = os.path.join(opt.save_dir, f"EMAgenerated_ep{ep}_w{w}_edm_steps{steps}_eta{eta}")
        else:
            if mode == 'DDPM':
                gen_dir = os.path.join(opt.save_dir, f"EMAgenerated_ep{ep}_w{w}_ddpm")
            else:
                gen_dir = os.path.join(opt.save_dir, f"EMAgenerated_ep{ep}_w{w}_ddim_steps{steps}_eta{eta}")
        os.makedirs(gen_dir)
